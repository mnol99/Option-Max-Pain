...
1. **Data source: Jupiter Perps (Doves oracle)**
   ...
   - **IBIT / BLK:** `IBIT_EXTRA_TXIDS` — comma-separated Bitcoin txids to always evaluate (e.g. Arkham transfers not yet in watch-address history). Signals must be on **today’s ET calendar day** (same clock time on an old date no longer fires). Set `IBIT_ALLOW_HISTORICAL_SIGNALS=1` only to replay past txs for testing. Daytime transfers **≥10:00 ET** use **6 covers 3:00–3:50pm ET** (paper sim); earlier signals use **18 covers 10:00–12:50 ET**.
