...
3. **Trade window**
   ...
   - **Warmup without waiting:** If **`BIRDEYE_API_KEY`** is set in `.env.local`, the server can **backfill** the last **4 closed** 60m candles (and **4 synthetic ET daily** bars built from hourly Birdeye data) on first tick so pattern detection can run immediately. Live prices still use **Doves / Pyth**. Disable with **`INSIDE_BAR_DISABLE_BIRDEYE_BACKFILL=1`**. If ETH hourly returns empty, set **`BIRDEYE_ETH_MINT`** to your token’s mint on Solana.
